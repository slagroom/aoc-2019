...
func (self *intcodeProcessor) valArg(position uint32) int32 {
	...
	switch mode {
	case 0: return self.fetch(uint32(value)) // position mode
...
func (self *intcodeProcessor) refArg(position uint32) uint32 {
	mode := self.argMode(position)
	switch mode {
		case 0: return uint32(self.fetch(self.programCounter + position))
		default:
...
func (self *intcodeProcessor) jnz() {
	...
	switch arg1 {
	case 0: self.programCounter += 3
	default: self.programCounter = arg2
	}
}
...
func (self *intcodeProcessor) jz() {
	...
	arg2 := uint32(self.valArg(2));
	switch arg1 {
	case 0: self.programCounter = arg2
	default: self.programCounter += 3
	}
}
...
func (self *intcodeProcessor) lt() {
    ...
    dst := self.refArg(3);
	switch arg1 < arg2 {
	case true: self.Store(dst, 1)
	case false: self.Store(dst, 0)
	}
    self.programCounter += 4
}
